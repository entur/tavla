            ...
            {deviations.map((deviation) =>
                deviation.cancelled ? (
                    <TableCell key={deviation.key}>
                        <div
                            className={`flex w-8 items-center justify-center text-error ${deviation.isVisible ? '' : 'opacity-50'}`}
                        >
                            <ValidationErrorFilledIcon size="0.8em" />
                        </div>
                    </TableCell>
                ) : (
                    <TableCell key={deviation.key}>
                        <div
                            className={`flex w-8 items-center justify-center text-warning ${deviation.isVisible ? '' : 'opacity-50'}`}
                        >
                            {deviation.situations.length > 0 ? (
                                <ValidationExclamationCircleFilledIcon size="0.8em" />
                            ) : (
                                <div />
                            )}
                        </div>
