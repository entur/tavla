...
export function TileList({
    ...
    const moveItem = (index: number, direction: string) => {
        ...
        // Keep the parent board in sync so the live preview reflects the new
        // order immediately. For real boards the order is also persisted
        // server-side; the demo board only lives in localStorage.
        setTilesLocalStorageBoard?.(newArray)
        if (board.id !== LOCAL_STORAGE_BOARD_ID) {
